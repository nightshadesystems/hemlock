...
/*
 * hemlock port: devm_nvmem_unregister() was removed in 5.18, and this
 * function re-registers when dev_class changes — so use the non-devm
 * API on every kernel and unregister explicitly in optoe_remove().
 */
	if (optoe->nvmem)
		nvmem_unregister(optoe->nvmem);
	optoe->nvmem = nvmem_register(&nvmem_config);
	if (IS_ERR(optoe->nvmem)) {
		int err = PTR_ERR(optoe->nvmem);
...
#endif
			if (IS_ERR_OR_NULL(optoe->optoe_dummy.client)) {
				dev_err(&client->dev,
