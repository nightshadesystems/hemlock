...
/* Hemlock local port; see kmod/README.md. */
...
static int as4610_psu_do_probe(struct i2c_client *client,
...
/* See the matching comment in accton_as4610_cpld.c. */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)
static int as4610_psu_probe(struct i2c_client *client)
{
	return as4610_psu_do_probe(client, i2c_client_get_device_id(client));
}
#endif

/* .remove has returned void since 6.1 (the floor asserted above). */
static void as4610_psu_remove(struct i2c_client *client)
{
...
/* .class/.address_list dropped: the i2c core only consults them from
 * i2c_detect(), which needs a .detect callback this driver never had, so
 * they were dead weight - and class-based instantiation is on its way
 * out upstream. Both PSU clients are created from the manifest topology. */
static struct i2c_driver as4610_psu_driver = {
	...
	.probe_new	  = as4610_psu_probe,
#else
	.probe		  = as4610_psu_do_probe,
#endif
